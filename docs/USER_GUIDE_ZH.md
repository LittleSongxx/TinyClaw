...
- 默认本地管理后台起始端口：`18080`
- 应用数据库：SQLite
- Agent / RAG v2 底座：PostgreSQL `pgvector` + Redis + MinIO
...
- MCP / Function Call / Skills / RAG / Cron 等扩展能力
...
- [`.env`](/path/to/TinyClaw/deploy/docker/.env)
  作用：运行配置文件，里面放平台凭据、模型类型、端口起始值，以及 AMap / Bocha / GitHub 等 MCP 密钥
- [docker-compose.yml](/path/to/TinyClaw/deploy/docker/docker-compose.yml)
  作用：容器编排文件
- [mcp.json](/path/to/TinyClaw/conf/mcp/mcp.json)
  作用：默认 MCP 配置文件，当前仓库会从这里加载 `playwright / filesystem / fetch / time / memory / arxiv / amap / bocha-search / github`
...
- [status.sh](/path/to/TinyClaw/scripts/status.sh)
  作用：查看容器状态和当前端口
- [verify.sh](/path/to/TinyClaw/scripts/verify.sh)
  作用：对健康检查、Admin、Agent API 和 RAG v2 做在线自检
...
- [http/http.go](/path/to/TinyClaw/http/http.go)
  Web API 入口
- [skill](/path/to/TinyClaw/skill)
  技能目录加载、校验和内置 fallback 逻辑
- [skills](/path/to/TinyClaw/skills)
  当前仓库自带的本地技能定义目录
...
- 首次启动时自动选择空闲端口
- 如果当前栈已经在运行，则复用当前 `HTTP/Admin` 端口
- 先拉起 `postgres / redis / hf-embeddings / etcd / minio / milvus / playwright-mcp`
- 再单独重建并替换 `app`
...
- 这个脚本默认不会停掉容器
- 它只会提醒当前栈仍在运行，并输出容器状态
- 这样可以避免误把已经配置好自启动的环境整个停掉

如果你明确要主动停止整套 Compose：
...
- 默认自检会检查 `/pong`、`/metrics`、Admin 首页、`/run/list`、RAG collection / jobs / retrieval debug
- `--full` 还会额外跑一轮真实的 `/task`、`/mcp` 和 `/run/replay`
- `--full` 会真实调用模型与浏览器工具，耗时和 token 开销都会更高

### 自启动说明

这套 Compose 部署默认已经启用了容器自启动策略：

- [docker-compose.yml](/home/song/code/Agent/TinyClaw/deploy/docker/docker-compose.yml) 中的服务统一使用 `restart: unless-stopped`
- 因此只要成功执行过一次 [start.sh](/home/song/code/Agent/TinyClaw/scripts/start.sh)，后续 Docker 守护进程恢复后会自动拉起 TinyClaw 相关容器
- 同一个仓库拿到其他机器上部署时，也会直接继承这套行为

补充说明：

- 如果是普通 Linux 服务器，建议确认 Docker 服务本身也是开机自启：

```bash
sudo systemctl enable --now docker
```

- 如果是 Docker Desktop 环境，则由 Docker Desktop 的开机启动设置决定守护进程是否自动恢复
...
- Dashboard
  看整体运行情况
- Runs
  看 Agent 运行轨迹、步骤、tool observation、replay，并支持单条删除和批量删除
- Bots
  看 Bot 列表和配置
- BotUsers
  看用户信息
...
- MCP
  管理 MCP 配置、模板和可用性检查
- Skills
  查看技能目录，区分 local / builtin / legacy，支持搜索、校验、reload 和详情查看
- Cron
  管理定时任务
- RAG
  通过 `Documents / Ingestion Jobs / Retrieval Debug` 三个页签管理知识数据
...
这通常表示某个 MCP 服务当前不可用，常见原因有：

- 对应服务本身没启动
- `conf/mcp/mcp.json` 里的地址或命令不对
- 你还没填需要的密钥，例如 `AMAP_MAPS_API_KEY`、`BOCHA_API_KEY`、`GITHUB_PERSONAL_ACCESS_TOKEN`

如果你当前只需要基础聊天，它通常不是飞书 + Qwen 主链路的阻塞问题；如果你要用 `/mcp`、Skills 或后台 `MCP / Skills` 页面，就需要把这些问题补齐。
